...
function OfflineTable(props) {
  ...
  function handlePageChange(pageNumber) {
    ...
    if (filterValue && filterValue.length > 0) {
      ...
      const end = filteredData.length > (skip + size) ? skip + size : filteredData.length
  ...
  function filterData(e) {
    ...
    if (filteredData.length > 0) {
      viewData = filteredData.slice(0, filteredData.length > 10 ? filteredData.length : 10)
    }
  ...
  function sort(index) {
    ...
    newData.sort = newData.sort === "sorting_asc" ? "sorting_desc" : "sorting_asc"
    headerFields[index] = newData

    var sortedData;
    if (filterValue.length) {
      sortedData = sortByColumn(filteredData, index, newData.sort === "sorting_asc")
  ...
  function renderHeader() {
    ...
    header.push(headerFields.map((row, idx) =>
      <th role="row"
        key={idx}
        className={` ${row.align ? row.align : ""} ${('sort' in row) ? row.sort : ""}`}
        // className={row.sort}
        tabIndex={idx}
  ...
  function onClickExport() {
    ...
    axios.get(EXPORT, { headers }).then(res => {
    ...
    return (
      viewData.map((row, index) => {
        ...
        if (props.showActions) {
          thisRow.push(<td className="text-center">
            {
              props.actions.map((action, actionIdx) => <i key={actionIdx} className={action.icon} onClick={() => action.callBack(row)} />)
            }
          </td>)
        }

        thisRow.push(row.map((col, subindex) => {
          var alignClass = headerFields[subindex].align ? headerFields[subindex].align : ""

          if (headerFields[subindex].type === "action") {

            thisRow.push(<td className="text-center">
              {
                col.map((action, actionIdx) => <i key={actionIdx} className={action.icon} onClick={() => action.callBack(row)} />)
              }
            </td>)

          }
          else if (headerFields[subindex].type === "badge") {
            return (<td className={"text-nowrap " + alignClass}> <span key={subindex} className={col.class}>{col.data}</span></td >)
          }
          else if (headerFields[subindex].type === "text") {
            return (<td className={alignClass} key={subindex}>{col}</td>)
          }
            ...
            var defaultProfilePicture = <button className="btn rounded-round btn-icon btn-lg" style={style}>
              <span className="letter-icon text-uppercase text-white">{initials}</span>
            </button>;

            if (col.picture !== null && col.picture !== '' && typeof col.picture !== 'undefined') {
              defaultProfilePicture = <img src={col.picture} onError={(e) => { e.target.onerror = null; e.target.src = defaultProfilePicture }} className="rounded-circle" width="40" height="40" alt="" />
            }
            return (
              <td className={alignClass}>
                <div className="media">
                  ...
                  <div className="media-body">
                    <div className="font-weight-semibold text-capitalize">{col.topText}</div>
                    <span className="text-muted font-size-sm">{col.bottomText}</span>
                  </div>
                </div>
              </td>
            )
            ...
            return (
              <td className="text-nowrap">
                <div className="list-icons-item dropdown">
                  <a href="#" className="list-icons-item dropdown-toggle" style={{ "color": "#22a2b1" }} data-toggle="dropdown" aria-expanded="false">{col.text}</a>
                  <div className={`dropdown-menu ${dropdownClass}`} x-placement="bottom-start" style={{ position: "absolute", willChange: "transform: top: 0px; left: 0px", transform: "translate3d(0px, 16px, 0px)" }}>
                    {
                      col.options.map((status, idx) => {
                        var __style = { "color": "#22a2b1" }
                        var __text = <i className="ml-auto icon-checkmark2" style={__style}></i>
                        if (status.value != col.selected) {
                          __style = {}
                          __text = ""
                        }
                        return <span key={'status -' + idx} onClick={col.onClick.bind(this, status.value, col.onClickArg)} className="dropdown-item" style={__style}> {status.label} {__text} </span>
                      })
                    }
                  </div>
                </div>
        ...
        <div className="datatable-header row">
          ...
          <div className="dataTables_filter" title="Filter data">
            <label>
              ...
              <input type="search" value={filterValue} placeholder={"Type to filter..."} onChange={(e) => { filterData(e) }} />
            </label>
  ...
  if (filterValue.length > 0) {
    viewData = filteredData.slice(skip, skip + size > filteredData.length ? filteredData.length : size)
  } else {
    viewData = data.slice(skip, skip + size)
  }

  var showingTo = skip + (filterValue.length > 0 ? filteredData.length : (viewData.length < (activePage * size) ? viewData.length : size));
  return (
    <div >
      ...
      <div className="row c-mt10 m-1">
        <div className="col-sm-12 col-md-6">
          <div className="records-info" role="status" aria-live="polite">
            {`Showing ${skip + 1} to ${showingTo} of ${filterValue.length > 0 ? filteredData.length : data.length} records`}
          </div>
        </div>
        <div className="col-sm-12 col-md-6 pagination-parent c-mt5r">
          <Pagination
            ...
            totalItemsCount={filterValue.length > 0 ? filteredData.length : data.length}
